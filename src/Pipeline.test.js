...
test('Create a pipeline', async () => {
  ...
  pipelineResource.addTransform(pipelineResource.transformTypes.TABULAR, {
    key1: 'value1',
  });
  ...
  expect(pipelineResource2.getTransformType(transforms[0])).toBe(
    pipelineResource2.transformTypes.TABULAR
  );
});
...
test('Check pipeline API', () => {
  ...
  const tr = pipeline.addTransform(pipeline.transformTypes.TABULAR, {
    key1: 'val1',
  });
...
test('Set and update arguments of pipeline', async () => {
  ...
  const configurationArgs =
    pipelineResource2.getPipelineArguments('configuration');
